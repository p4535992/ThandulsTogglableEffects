import { log } from '../ThandulsTogglableEffects.js';
...
export async function rollFromCompendium (item, event) {
  ...
  if (item instanceof Actor) return rollSimple(item,null)
  if (item instanceof Scene) return rollSimple(item,null)
...
export async function rollItem (item, event) {
  ...
  const actor = getCanvas().tokens.controlled[0]?.actor || dummyActor
